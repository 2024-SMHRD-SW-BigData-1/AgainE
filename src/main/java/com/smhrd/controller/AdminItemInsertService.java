...
public class AdminItemInsertService extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			e.printStackTrace();
			response.sendRedirect("boardWrite.java");
			return;
		}
		
		String item_name = multi.getParameter("item_name");
		String item_tone = multi.getParameter("item_tone");
		...
		Integer item_price = Integer.parseInt(multi.getParameter("item_price"));
		String item_url = multi.getFilesystemName("item_url");
		...
		int cnt = dao.insertAdminItems(item);

		if (cnt > 0) {
			showAlert(response, "success", "추가 성공", "", "AdminItemListService");
		} else {
			String referer = request.getHeader("Referer");
			showAlert(response, "error", "추가 실패", "", referer);
		}
	}
